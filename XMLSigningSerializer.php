	...
	class XMLSigningSerializer {
		...
		function serializeString( $string, $isTextNode )
		{
			...
			if( $len == 0 )
			{
				// empty string
				if ($this->verbose)
					echo "<p>String: $string --- length is zero</p>";
				$outString = $nodeID.chr(0).chr(0);//$nodeId . chr(0) . chr(0);
			} 
			else
			{
				if( $len > 0x7FFF )
			    ...
			    else
			    {
					if ( $isTextNode )
					{
						$outString = $this->TEXT_NODE.chr(0) . chr(0) . $outString;
						$outString[1] = chr(floor( $len / 256 ));
						$outString[2] = chr( $len % 256 );                                 
					}
					else
					{
